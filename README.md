...
- The NeoForge browser uses Minecraft GUI coordinates directly instead of multiplying the viewport by `guiScale`, keeps MCEF's persistent texture/dirty-rectangle cache, renders the full-screen surface as opaque, and does not poll diagnostics while the page is idle. This avoids redundant high-DPI pixels and self-triggered idle repaint loops.
- `config/mcwebui-client.toml` exposes `followGuiSize` (default `true`). Set it to `false` and reopen the F8 screen to lock the browser viewport at its opening size; later window/GUI-size changes stretch the cached texture and map X/Y input independently back into the locked viewport.
