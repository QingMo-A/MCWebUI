...
The Windows GAME_SYNC native checkpoint is recorded in [`plans/frame-pacing-plan.md`](plans/frame-pacing-plan.md). Exact source patches now expose an opt-in External Begin Frame creation path and the Windows amd64 JCEF JNI wrapper builds against the pinned CEF 116/5845 SDK without rebuilding Chromium. Stock mode remains unchanged. A matched patched MCEF NeoForge artifact and client benchmark are still unavailable, so the result is **PROOF C**, not a runtime FPS claim; GAME_SYNC remains experimental and off by default.
