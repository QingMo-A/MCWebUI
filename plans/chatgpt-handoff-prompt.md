...
8. The GAME_SYNC/frame-pacing audit is captured in `plans/frame-pacing-plan.md`. CEF's native `SetWindowlessFrameRate`/`SendExternalBeginFrame` concepts require a browser host/creation path absent from the bundled CinemaMod Java/JNI API; no game-tick repaint loop, private JNI, binary patch, or fabricated FPS result is permitted. Re-open only after a supported wrapper and paired runtime benchmark are available.
