...
The target already inherits MCEF's persistent OpenGL texture and dirty-rectangle upload behavior, so MCWebUI does not retain the callback-owned CEF `ByteBuffer` or add a second full-frame cache. The remaining avoidable work was an oversized viewport and a self-sustaining diagnostics repaint loop. `NeoForgeWebSession` now uses Minecraft GUI dimensions directly, the browser is opaque, the Screen draws without alpha blending, and playground diagnostics refresh only after connection, when entering Runtime, or on explicit request. With `guiScale=2`, using GUI rather than framebuffer dimensions reduces the browser pixel count and full RGBA surface estimate by 75%; the current local acceptance window used `guiScale=1`, where that particular reduction is intentionally zero.

NeoForge now registers `config/mcwebui-client.toml` with `followGuiSize=true` by default. `false` locks the viewport created when F8 opens, keeps the Screen quad fitted to the current GUI, and maps X/Y independently from current GUI coordinates into the locked browser coordinates. Non-square resize mapping is unit-tested. Diagnostics report `followGuiSize` and `viewportMode`; the shared showcase displays the mode without polling while idle. Both config values completed real NeoForge/MCEF startup and F8 rendering, followed by clean shutdown. CEF documents a default 30 FPS maximum for windowless paint callbacks, but the MCEF/JCEF 2.1.6 Java surface used here exposes no public frame-rate setter; no reflective/native workaround is claimed.
...
NeoForge JAR audit (local): `targets/neoforge-1.21.1/build/libs/neoforge-1.21.1-0.1.0-SNAPSHOT.jar` SHA-256 `71CF4A57B81DD501E43433F77F3D8FE2CB29B63CEAA5939F77D87211C6D9419A`. The packaged `index.html` (`D55B8D5A001D13F64C44AA8DFE814EFD376BB38C2617E8B0D6A14B3A878669AF`), JS (`B0A43CAD5267F6C7196499AB961A51CEE6B2954903AB9ED48CDDF81DE9C373EC`), and CSS (`B70AB76C3022E19FE560908DC9CEC8232BE0EBE6368A2F329987D7AF35BB8117`) hashes match the single `frontend/playground/dist` source. The JAR has 85 entries, contains common runtime classes exactly once and three web files, and has no duplicate entries, Forge adapter, frontend source, source map, or node_modules content.
