...
The NeoForge adapter registers `mcui` through JCEF `onRegisterCustomSchemes` before CEF initialization. It is a standard, non-local, secure, CORS-enabled, fetch-enabled, display-isolated scheme. Packaged namespaces use qualified hosts such as `playground.mcwebui`; the handler strips the reserved `.mcwebui` suffix, rejects traversal/unknown hosts, buffers each classpath resource, and reports its exact response length. The browser's minimal `{version,type:"handshake"}` negotiation envelope intentionally omits host capabilities; `BridgeCodec` accepts that request and the host response supplies the negotiated set.
